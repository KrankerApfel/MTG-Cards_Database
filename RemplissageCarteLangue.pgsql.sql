...
('Angel of Invention', 'Flying, vigilance, lifelink Fabricate 2 (When this creature enters the battlefield, put two +1/+1 counters on it or create two 1/1 colorless Servo artifact creature tokens.) Other creatures you control get +1/+1.', 87, 1),
  ('Authority of the Consuls', 'Creatures your opponents control enter the battlefield tapped. Whenever a creature enters the battlefield under an opponent''s control, you gain 1 life.', 88, 1),
  ('Aviary Mechanic', 'When Aviary Mechanic enters the battlefield, you may return another permanent you control to its owner''s hand.', 89, 1),
  ('Built to Last', 'Target creature gets +2/+2 until end of turn. If it''s an artifact creature, it gains indestructible until end of turn. (Damage and effects that say "destroy" don''t destroy it.)', 90, 1),
  ('Captured by the Consulate', 'Enchant creature you don''t control Enchanted creature can''t attack. Whenever an opponent casts a spell, if it has a single target, change the target to enchanted creature if able.', 91, 1),
  ('Cataclysmic Gearhulk', 'When Cataclysmic Gearhulk enters the battlefield, each player chooses an artifact, a creature, an enchantment, and a planeswalker from among the nonland permanents he or she controls, then sacrifices the rest.', 92, 1),
  ('Consulate Surveillance', 'When Consulate Surveillance enters the battlefield, you get {E}{E}{E}{E} (four energy counters). Pay {E}{E}: Prevent all damage that would be dealt to you this turn by a source of your choice.', 93, 1),
  ('Consul''s Shieldguard', 'When Consul''s Shieldguard enters the battlefield, you get ee (two energy counters). Whenever Consul''s Shieldguard attacks, you may pay e. If you do, another target attacking creature gains indestructible until end of turn.', 94, 1),
  ('Eddytrail Hawk', 'When Eddytrail Hawk enters the battlefield, you get ee (two energy counters). Whenever Eddytrail Hawk attacks, you may pay e. If you do, another target attacking creature gains flying until end of turn.', 95, 1),
  ('Fairgrounds Warden', 'When Fairgrounds Warden enters the battlefield, exile target creature an opponent controls until Fairgrounds Warden leaves the battlefield.', 96, 1),
  ('Fragmentize', 'Destroy target artifact or enchantment with converted mana cost 4 or less.', 97, 1),
  ('Aegis of Honor', '1: The next time an instant or sorcery spell would deal damage to you this turn, that spell deals that damage to its controller instead. ', 1, 1),
  ('Ancestral Tribute', 'You gain 2 life for each card in your graveyard. Flashback 9WWW (You may play this card from your graveyard for its flashback cost. Then remove it from the game.) ', 2, 1),
  ('Angelic Wall', '(Walls can''t attack.) Flying ', 3, 1),
  ('Animal Boneyard', 'Enchanted land has "Tap, Sacrifice a creature: You gain life equal to that creature''s toughness." ', 4, 1),
  ...
  ('Tireless Tribe', 'Discard a card from your hand: Tireless Tribe gets +0/+4 until end of turn. ', 56, 1),
  ('Wayward Angel', 'Flying Attacking doesn''t cause Wayward Angel to tap. Threshold - Wayward Angel gets +3/+3, is black, has trample, and has "At the beginning of your upkeep, sacrifice a creature". (You have threshold as long as seven or more cards are in your graveyard.) ', 57, 1),
  ('Angel of Retribution', 'Flying, first strike', 71, 1),
  ('Aven Trooper', 'Flying. 2W, Discard a card from your hand: Aven Trooper gets+1/+2 until end of turn.', 72, 1),
  ('Cleansing Meditation', 'Destroy all enchantments. Threshold - Instead destroy all enchantments, then return to play all cards in your graveyard destroyed this way. (You have threshold if seven or more cards are in your graveyard.)', 73, 1),
  ('Equal Treatment', 'If any source would deal 1 or more damage to a creature or player this turn, it deals 2 damage to that creature or player instead. Draw a card.', 74, 1),
  ('Floating Shield', 'As Floating Shield comes into play, choose a color. Enchanted creature has protection from the chosen color. This effect doesn''t remove Floating Shield. Sacrifice Floating Shield: Target creature gains protection from the chosen color until end of turn.', 75, 1),
  ('Frantic Purification', 'Destroy target enchantment. Madness W (You may play this card for its madness cost at the time you discard it from your hand.)', 76, 1),
  ('Hypochondria', 'W, Discard a card from your hand: Prevent the next 3 damage that would be dealt to target creature or player this turn. W, Sacrifice Hypochondria: Prevent the next 3 damage that would be dealt to target creature or player this turn.', 77, 1),
  ('Major Teroh', 'Flying. 3WW, Sacrifice Major Teroh: Remove all black creatures from the game.', 78, 1),
  ('Militant Monk', 'Attacking doesn''t cause Militant Monk to tap. TAP: Prevent the next 1 damage that would be dealt to target creature or player this turn.', 79, 1),
  ('Morningtide', 'Remove all cards in all graveyards from the game.', 80, 1),
  ('Mystic Familiar', 'Flying. Threshold - Mystic Familiar gets +1/+1 and has protection from black. (You have threshold as long as seven or more cards are in your graveyard.)', 81, 1),
  ('Pay No Heed', 'Prevent all damage a source of your choice would deal this turn.', 82, 1),
  ('Possessed Nomad', 'Attacking doesn''t cause Possessed Nomad to tap. Threshold - Possessed Nomad gets +1/+1, is black, and has "2B, TAP: Destroy target white creature." (You have threshold as long as seven or more cards are in your graveyard.)', 83, 1),
  ('Manoeuvre acrobatique', 'Exilez une créature ciblée que vous contrôlez, puis renvoyez cette carte sur le champ de bataille sous le contrôle de son propriétaire. Piochez une carte.', 84, 2),
  ('Secoureur aérien', 'Vol, vigilance, lien de vie', 85, 2),
  ('Rokh tempéther', 'À chaque fois que le Rokh tempéther ou une autre créature arrive sur le champ de bataille sous votre contrôle, vous gagnez e (un marqueur « énergie »). À chaque fois que le Rokh tempéther attaque, vous pouvez payer ee. Si vous faites ainsi, mettez un marqueur +1/+1 sur lui et engagez jusqu''à une créature ciblée que le joueur défenseur contrôle.', 86, 2),
  ('Ange de l’inventivité', 'Vol, vigilance, lien de vie Fabrication 2 (Quand cette créature arrive sur le champ de bataille, mettez deux marqueurs +1/+1 sur elle ou créez deux jetons de créature-artefact 1/1 incolore Servo.) Les autres créatures que vous contrôlez gagnent +1/+1.', 87, 2),
  ('Autorité des consuls', 'Les créatures que vos adversaires contrôlent arrivent sur le champ de bataille engagées. À chaque fois qu''une créature arrive sur le champ de bataille sous le contrôle d''un adversaire, vous gagnez 1 point de vie.', 88, 2),
  ('Mécanicienne de la volière', 'Quand la Mécanicienne de la volière arrive sur le champ de bataille, vous pouvez renvoyer un autre permanent que vous contrôlez dans la main de son propriétaire.', 89, 2),
  ('Construit pour durer', 'La créature ciblée gagne +2/+2 jusqu''à la fin du tour. Si c''est une créature-artefact, elle acquiert l''indestructible jusqu''à la fin du tour. (Les blessures et les effets qui disent « détruisez » ne la détruisent pas.)', 90, 2),
  ('Capturée par le consultant', 'La créature enchantée ne peut pas attaquer. À chaque fois qu''un adversaire lance un sort, s''il a une seule cible, remplacez la cible par la créature enchantée si possible.', 91, 2),
  ('Carcasse mécanique cataclysme', 'Quand la Carcasse mécanique cataclysmique arrive sur le champ de bataille, chaque joueur choisit un artefact, une créature, un enchantement et un planeswalker parmi les permanents non-terrain qu''il contrôle, puis sacrifie le reste.', 92, 2),
  ('Surveillance du consulat', 'Quand la Surveillance du consulat arrive sur le champ de bataille, vous gagnez eeee (quatre marqueurs « énergie »). Payez ee : Prévenez toutes les blessures qui devraient vous être infligées ce tour-ci par une source de votre choix.', 93, 2),
  ('Gardebouclier du consul', 'Quand le Gardebouclier du consul arrive sur le champ de bataille, vous gagnez ee (deux marqueurs « énergie »). À chaque fois que le Gardebouclier du consul attaque, vous pouvez payer e. Si vous faites ainsi, une autre créature attaquante ciblée acquiert l''indestructible jusqu''à la fin du tour.', 94, 2),
  ('Faucon des courants', 'Quand le Faucon des courants arrive sur le champ de bataille, vous gagnez ee (deux marqueurs « énergie »). À chaque fois que le Faucon des courants attaque, vous pouvez payer e. Si vous faites ainsi, une autre créature attaquante ciblée acquiert le vol jusqu''à la fin du tour.', 95, 2),
  ('Garde de la foire', 'Quand le Garde de la foire arrive sur le champ de bataille, exilez une créature ciblée qu''un adversaire contrôle jusqu''à ce que le Garde de la foire quitte le champ de bataille.', 96, 2),
  ('Fragmentation', 'Détruisez l''artefact ou l''enchantement avec un coût converti de mana inférieur ou égal à 4 ciblé.', 97, 2),
  ('Acrobatic Maneuver', 'Exile target creature you control, then return that card to the battlefield under its owner''s control. Draw a card.', 84, 1),
  ('Aerial Responder', 'Flying, vigilance, lifelink', 85, 1),
  ('Aetherstorm Roc', 'Whenever Aetherstorm Roc or another creature enters the battlefield under your control, you get {E} (an energy counter). Whenever Aetherstorm Roc attacks, you may pay {E}{E}. If you do, put a +1/+1 counter on it and tap up to one target creature defending player controls.', 86, 1);
